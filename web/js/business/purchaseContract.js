...
function totalPage() {
    ...
    if (!isSearch) {
        $.ajax({
            type: "POST",                       // 方法类型
            url: "loadPagePurchaseContractListCount",                  // url
        ...
        $.ajax({
            type: "POST",                       // 方法类型
            url: "searchPurchaseContractCount",                  // url
            async: false,                      // 同步：意思是当有返回值以后才会进行后面的js程序
...
function setPageClone(result) {
    $(".beforeClone").remove();
    setPurchaseContractList(result);
    var total = totalPage();
    $("#next").prev().hide();
    var st = "共" + total + "页";
    $("#totalPage").text(st);
...
function switchPage(pageNumber) {
    ...
    if (!isSearch) {
        $.ajax({
            type: "POST",                       // 方法类型
            url: "loadPagePurchaseContractList",         // url
            async: false,                      // 同步：意思是当有返回值以后才会进行后面的js程序
        ...
        $.ajax({
            type: "POST",                       // 方法类型
            url: "searchPurchaseContract",         // url
            async: false,                      // 同步：意思是当有返回值以后才会进行后面的js程序
            data: JSON.stringify(data),
            dataType: "json",
            contentType: 'application/json;charset=utf-8',
 ...
 * 分页 获取首页内容
 * */
function loadPagePurchaseContractList() {
 ...
 * @param result
 */
function setPurchaseContractList(result) {
...
function addContract() {
    localStorage.contractType="Purchase"
