...
function totalPage() {
    ...
    if (!isSearch) {
        $.ajax({
            type: "POST",                       // 方法类型
            url: "loadPageEmergencyContractListCount",                  // url
        ...
        $.ajax({
            type: "POST",                       // 方法类型
            url: "searchEmergencyContractCount",                  // url
            async: false,                      // 同步：意思是当有返回值以后才会进行后面的js程序
...
function setPageClone(result) {
    $(".beforeClone").remove();
    setEmergencyContractList(result);
    var total = totalPage();
    $("#next").prev().hide();
    var st = "共" + total + "页";
    $("#totalPage").text(st);
...
function switchPage(pageNumber) {
    ...
    if (!isSearch) {
        ...
        $.ajax({
            type: "POST",                       // 方法类型
            url: "searchEmergencyContract",         // url
            async: false,                      // 同步：意思是当有返回值以后才会进行后面的js程序
            data: JSON.stringify(data),
            dataType: "json",
            contentType: 'application/json;charset=utf-8',
 ...
 * 分页 获取首页内容
 * */
function loadPageEmergencyContractList() {
    ...
    if(getApprovalId()!=undefined){ //存在
        ...
        searchEmergencyContract();
        window.localStorage.removeItem('approvalId');
    }else {
        $.ajax({
            type: "POST",                       // 方法类型
            url: "loadPageEmergencyContractList",          // url
            async: false,                       // 同步：意思是当有返回值以后才会进行后面的js程序
            data: JSON.stringify(page),
            dataType: "json",
            contentType: 'application/json;charset=utf-8',
 ...
 * @param result
 */
function setEmergencyContractList(result) {
 ...
 * 查找客户
 */
function searchEmergencyContract() {
    isSearch = true;
    var page = {};
    var pageNumber = 1;                       // 显示首页
    page.pageNumber = pageNumber;
...
function addContract() {
    localStorage.contractType="Emergency"
